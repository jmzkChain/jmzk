...
private:
    ...
    static void
    set_from_word_sequence(const std::array<Word, NumWords>& arr, fixed_key<Size>& key) {
...
public:
    ...
    num_words() {
        return (Size + sizeof(word_t) - 1) / sizeof(word_t);
    }
    static constexpr size_t
    padded_bytes() {
        return num_words() * sizeof(word_t) - Size;
    }
    ...
    static fixed_key<Size>
    make_from_word_sequence(
        typename std::enable_if<std::is_integral<FirstWord>::value && !std::is_same<FirstWord, bool>::value
...
template <size_t Size>
bool
operator==(const fixed_key<Size>& c1, const fixed_key<Size>& c2) {
