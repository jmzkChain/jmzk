...
#define SY(P,X) ::evt::chain::string_to_symbol_c(P,#X)
      ...
      class symbol {
         public:
            ...
            explicit symbol(uint64_t v = SY(4, EVT)): m_value(v) { }
            ...
            explicit operator string() const
            {
               uint64_t v = m_value;
               uint8_t p = v & 0xFF;
               string ret = evt::chain::to_string(p);
               ret += ',';
               ret += name();
               return ret;
            }
...
namespace fc {
   inline void to_variant(const evt::chain::symbol& var, fc::variant& vo) { vo = var.to_string(); }
   inline void from_variant(const fc::variant& var, evt::chain::symbol& vo) {
      vo = evt::chain::symbol::from_string(var.get_string());
   }
}

namespace fc {
   inline void to_variant(const evt::chain::symbol_code& var, fc::variant& vo) {
      vo = evt::chain::symbol(var.value << 8).name();
   }
   inline void from_variant(const fc::variant& var, evt::chain::symbol_code& vo) {
      vo = evt::chain::symbol(0, var.get_string().c_str()).to_symbol_code();
   }
}

FC_REFLECT(evt::chain::symbol_code, (value))
FC_REFLECT(evt::chain::symbol, (m_value))
