...
namespace fc {
  template<uint16_t IntervalMs, uint64_t EpochMs>
  void to_variant(const evt::chain::block_timestamp<IntervalMs,EpochMs>& t, fc::variant& v) {
     to_variant( (fc::time_point)t, v);
  }

  template<uint16_t IntervalMs, uint64_t EpochMs>
  void from_variant(const fc::variant& v, evt::chain::block_timestamp<IntervalMs,EpochMs>& t) {
