...
namespace evt { namespace chain { 
    ...
    void name128::set( const char* str ) {
        const auto len = strnlen(str, 22);
        EVT_ASSERT(len <= 21, name_type_exception, "Name128 is longer than 21 characters (${name}) ", ("name", string(str)));
...
namespace fc {
    void to_variant(const evt::chain::name& c, fc::variant& v) { v = std::string(c); }
    void from_variant(const fc::variant& v, evt::chain::name& check) { check = v.get_string(); }
    void to_variant(const evt::chain::name128& c, fc::variant& v) { v = std::string(c); }
    void from_variant(const fc::variant& v, evt::chain::name128& check) { check = v.get_string(); }
} // fc
