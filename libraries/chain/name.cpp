...
namespace eosio { namespace chain { 
    ...
    void name128::set( const char* str ) {
        ...
        EOS_ASSERT(len <= 21, name_type_exception, "Name128 is longer than 21 characters (${name}) ", ("name", string(str)));
...
namespace fc {
    ...
    void to_variant(const eosio::chain::name128& c, fc::variant& v) { v = std::string(c); }
    void from_variant(const fc::variant& v, eosio::chain::name128& check) { check = v.get_string(); }
} // fc
