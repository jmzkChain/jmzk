...
void
to_variant(const fc::path& path_to_convert, variant& variant_output) {
...
void
from_variant(const fc::variant& variant_to_convert, fc::path& path_output) {
...
bool
operator<(const fc::path& l, const fc::path& r) {
    return *l._p < *r._p;
}
...
path
operator/(const fc::path& p, const fc::path& o) {
    path tmp;
    tmp = *p._p / *o._p;
    return tmp;
}
...
std::wstring
path::preferred_wstring() const {
    return boost::filesystem::path(*_p).make_preferred().wstring();
}

std::string
path::to_native_ansi_path() const {
...
void
path::replace_extension(const fc::path& e) {
...
bool
operator==(const directory_iterator& r, const directory_iterator& l) {
    return *r._p == *l._p;
}
...
int
recursive_directory_iterator::level() {
    return _p->depth();
}

bool
operator==(const recursive_directory_iterator& r, const recursive_directory_iterator& l) {
    return *r._p == *l._p;
}
...
void
resize_file(const path& f, size_t t) {
    try {
...
// no-op on Windows.
void
chmod(const path& p, int perm) {
#ifndef WIN32
    mode_t actual_perm = ((perm & 0400) ? S_IRUSR : 0)
                         | ((perm & 0200) ? S_IWUSR : 0)
                         | ((perm & 0100) ? S_IXUSR : 0)
...
void
rename(const path& f, const path& t) {
    try {
...
void
create_hard_link(const path& f, const path& t) {
    try {
...
fc::path
make_relative(const fc::path& from, const fc::path& to) {
...
bool
simple_lock_file::impl::try_lock() {
#ifdef _WIN32
...
bool
simple_lock_file::try_lock() {
    return my->try_lock();
}

void
simple_lock_file::unlock() {
    my->unlock();
}
