...
to_asio_ep(const fc::ip::endpoint& e) {
...
void
gelf_appender::initialize(boost::asio::io_service& io_service) {
    try {
        try {
...
void
gelf_appender::log(const log_message& message) {
    if(!my->gelf_endpoint.has_value())
        return;
    ...
    if(gelf_message_as_string[1] == (char)0x01 || gelf_message_as_string[1] == (char)0xda)
