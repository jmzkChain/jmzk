...
Byte(unsigned int ui) {
    return (unsigned char)(ui & 0xff);
}

//Function F
inline unsigned int
blowfish::F(unsigned int ui) {
...
void
blowfish::start(unsigned char* ucKey, uint64_t keysize, const sblock& roChain) {
    m_oChain0 = roChain;
    m_oChain  = roChain;
...
inline void
BytesToBlock(unsigned char const* p, sblock& b) {
    unsigned int y;
    //Left
    b.m_uil = 0;
    y       = *p++;
...
inline void
BlockToBytes(sblock const& b, unsigned char* p) {
    unsigned int y;
    //Right
    y    = b.m_uir;
    *--p = Byte(y);
...
void
blowfish::encrypt(unsigned char* buf, uint64_t n, int iMode) {
...
//Returns false if n is multiple of 8
void
blowfish::decrypt(unsigned char* buf, uint64_t n, int iMode) {
    //Check the buffer's length - should be > 0 and multiple of 8
    if((n == 0) || (n % 8 != 0))
        FC_THROW_EXCEPTION(exception, "invalid buffer length ${n}, not multiple of 8", ("n", n));
    sblock work;
...
//Returns false if n is multiple of 8
void
blowfish::encrypt(const unsigned char* in, unsigned char* out, uint64_t n, int iMode) {
    //Check the buffer's length - should be > 0 and multiple of 8
    if((n == 0) || (n % 8 != 0))
        FC_THROW_EXCEPTION(exception, "invalid buffer length ${n}, not multiple of 8", ("n", n));
    sblock work;
...
//Returns false if n is multiple of 8
void
blowfish::decrypt(const unsigned char* in, unsigned char* out, uint64_t n, int iMode) {
    //Check the buffer's length - should be > 0 and multiple of 8
    if((n == 0) || (n % 8 != 0))
        FC_THROW_EXCEPTION(exception, "invalid buffer length ${n}, not multiple of 8", ("n", n));
    sblock work;
