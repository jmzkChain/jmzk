...
compact_signature
signature_from_ecdsa(const EC_KEY* key, const public_key_data& pub_data, fc::ecdsa_sig& sig, const fc::sha256& d) {
...
private_key
private_key::regenerate(const fc::sha256& secret) {
    private_key self;
...
signature
private_key::sign(const fc::sha256& digest) const {
...
bool
public_key::verify(const fc::sha256& digest, const fc::crypto::r1::signature& sig) {
...
bool
private_key::verify(const fc::sha256& digest, const fc::crypto::r1::signature& sig) {
    return 1 == ECDSA_verify(0, (unsigned char*)&digest, sizeof(digest), (unsigned char*)&sig, sizeof(sig), my->_key);
}

public_key
private_key::get_public_key() const {
    public_key pub;
...
fc::sha512
private_key::get_shared_secret(const public_key& other) const {
...
compact_signature
private_key::sign_compact(const fc::sha256& digest) const {
    try {
        FC_ASSERT(my->_key != nullptr);
