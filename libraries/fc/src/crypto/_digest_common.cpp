...
static void
shift_l(const uint8_t* in, uint8_t* out, std::size_t n, unsigned int i) {
    if(i < n) {
        memcpy(out, in + i, n - i);
    }
    else {
...
void
shift_l(const char* in, char* out, std::size_t n, unsigned int i) {
...
void
shift_r(const char* in, char* out, std::size_t n, unsigned int i) {
    const uint8_t* in8  = (uint8_t*)in;
    uint8_t*       out8 = (uint8_t*)out;
