...
sha512
sha512::hash(const char* d, uint32_t dlen) {
    encoder e;
    e.write(d, dlen);
    return e.result();
}
...
sha512
sha512::hash(const string& s) {
    return hash(s.c_str(), s.size());
}

void
sha512::encoder::write(const char* d, uint32_t dlen) {
    SHA512_Update(&my->ctx, d, dlen);
}
sha512
sha512::encoder::result() {
    sha512 h;
...
void
sha512::encoder::reset() {
    SHA512_Init(&my->ctx);
}

sha512
operator<<(const sha512& h1, uint32_t i) {
    sha512 result;
...
sha512
operator^(const sha512& h1, const sha512& h2) {
    sha512 result;
...
void
to_variant(const sha512& bi, variant& v) {
    v = std::vector<char>((const char*)&bi, ((const char*)&bi) + sizeof(bi));
}

void
from_variant(const variant& v, sha512& bi) {
...
template<>
unsigned int
hmac<sha512>::internal_block_size() const {
    return 128;
}
