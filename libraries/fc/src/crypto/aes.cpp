...
void
aes_encoder::init(const fc::sha256& key, const fc::uint128& init_value) {
...
uint32_t
aes_encoder::encode(const char* plaintxt, uint32_t plaintext_len, char* ciphertxt) {
...
void
aes_decoder::init(const fc::sha256& key, const fc::uint128& init_value) {
    my->ctx.obj = EVP_CIPHER_CTX_new();
    /* Create and initialise the context */
    if(!my->ctx) {
        FC_THROW_EXCEPTION(aes_exception, "error allocating evp cipher context",
...
unsigned
aes_cfb_decrypt(unsigned char* ciphertext, int ciphertext_len, unsigned char* key,
                unsigned char* iv, unsigned char* plaintext) {
    evp_cipher_ctx ctx(EVP_CIPHER_CTX_new());
    int            len           = 0;
    unsigned       plaintext_len = 0;
...
std::vector<char>
aes_encrypt(const fc::sha512& key, const std::vector<char>& plain_text) {
...
std::vector<char>
aes_decrypt(const fc::sha512& key, const std::vector<char>& cipher_text) {
 ...
 */
void
aes_save(const fc::path& file, const fc::sha512& key, std::vector<char> plain_text) {
    try {
 ...
 */
std::vector<char>
aes_load(const fc::path& file, const fc::sha512& key) {
    try {
...
void
openssl_thread_config::set_thread_id(CRYPTO_THREADID* id) {
    CRYPTO_THREADID_set_numeric(id, std::hash<std::thread::id>()(std::this_thread::get_id()));
}

void
openssl_thread_config::locking_callback(int mode, int type, const char* file, int line) {
    if(mode & CRYPTO_LOCK)
...
openssl_thread_config::openssl_thread_config() {
    if(CRYPTO_THREADID_get_callback() == NULL && CRYPTO_get_locking_callback() == NULL) {
