...
sha1
sha1::hash(const char* d, uint32_t dlen) {
    encoder e;
    e.write(d, dlen);
    return e.result();
}
...
sha1
sha1::hash(const string& s) {
    return hash(s.c_str(), s.size());
}

void
sha1::encoder::write(const char* d, uint32_t dlen) {
...
sha1
operator<<(const sha1& h1, uint32_t i) {
    sha1 result;
...
sha1
operator^(const sha1& h1, const sha1& h2) {
    sha1 result;
...
void
to_variant(const sha1& bi, variant& v) {
    v = std::vector<char>((const char*)&bi, ((const char*)&bi) + sizeof(bi));
}

void
from_variant(const variant& v, sha1& bi) {
