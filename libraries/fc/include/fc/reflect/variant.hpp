...
private:
    template<typename M>
    void
    add(mutable_variant_object& vo, const char* name, const std::optional<M>& v) const {
        if(v.has_value()) {
            vo(name, *v);
        }
    }
    ...
    template<typename M>
    void add(mutable_variant_object& vo, const char* name, const M& v) const { vo(name, v); }
...
struct if_enum {
    template<typename T>
    static inline void
    to_variant(const T& v, fc::variant& vo) {
    ...
    template<typename T>
    static inline void
    from_variant(const fc::variant& v, T& o) {
...
struct if_enum<fc::true_type> {
    template<typename T>
    static inline void
    to_variant(const T& o, fc::variant& v) {
...
template<typename T>
void
to_variant(const T& o, variant& v) {
    if_enum<typename fc::reflector<T>::is_enum>::to_variant(o, v);
}

template<typename T>
void
from_variant(const variant& v, T& o) {
