...
fc::variant
parseInt(const std::string& token, size_t start) {
...
fc::variant
maybeParseInt(const std::string& token, size_t start) {
    try {
...
fc::variant
parseNumberOrStr(const std::string& token) {
    try {
        //ilog( (token) );
...
template<typename T, bool strict>
variants
arrayFromStream(T& in, uint32_t max_depth) {
    variants ar;
    try {
        if(in.peek() != '[')
