...
inline void
pack(Stream& s, const flat_set<T, Compare, Container>& value) {
...
template<typename Stream, typename T, typename Compare, typename Container>
inline void
unpack(Stream& s, flat_set<T, Compare, Container>& value) {
    unsigned_int size;
    unpack(s, size);
...
void
pack(Stream& s, const bip::vector<T, A>& value) {
    FC_ASSERT(value.size() <= MAX_NUM_ARRAY_ELEMENTS);
    pack(s, unsigned_int((uint32_t)value.size()));
...
template<typename Stream, typename T, typename A>
void
unpack(Stream& s, bip::vector<T, A>& value) {
    unsigned_int size;
    unpack(s, size);
    FC_ASSERT(size.value <= MAX_NUM_ARRAY_ELEMENTS);
    value.resize(size);
...
void
to_variant(const flat_set<T, Compare, Container>& var, variant& vo) {
...
template<typename T, typename Compare, typename Container>
void
from_variant(const variant& var, flat_set<T, Compare, Container>& vo) {
...
void
to_variant(const flat_map<K, T...>& var, variant& vo) {
    std::vector<variant> vars(var.size());
    size_t               i = 0;
    for(auto itr = var.begin(); itr != var.end(); ++itr, ++i)
...
void
from_variant(const variant& var, flat_map<K, T, A...>& vo) {
    const variants& vars = var.get_array();
    vo.clear();
    for(auto itr = vars.begin(); itr != vars.end(); ++itr)
