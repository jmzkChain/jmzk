...
public:
    ...
    on_incoming_block(const signed_block_ptr& block) {
        ...
        if(fc::time_point::now() - block->timestamp < fc::seconds(5) || (block->block_num() % 1000 == 0)) {
            ilog("Received block ${id}... #${n} @ ${t} signed by ${p} [trxs: ${count}, lib: ${lib}, confirmed: ${confs}]",
                 ("p", block->producer)
                 ("id", fc::variant(block->id()).as_string().substr(0, 16))
                 ("n", block_header::num_from_id(block->id()))
                 ("t", block->timestamp)
    ...
    on_incoming_transaction(const packed_transaction_ptr& trx) {
...
producer_plugin::plugin_initialize(const boost::program_options::variables_map& options) {
    try {
        ...
        my->_incoming_transaction_sync_provider = app().get_method<incoming::methods::transaction_sync>().register_provider(
            [this](const packed_transaction_ptr& trx) -> transaction_trace_ptr { 
      ...
      */
        ...
        case block_production_condition::produced: {
            ...
            ilog("Produced block ${id}... #${n} @ ${t} signed by ${p} [trxs: ${count}, lib: ${lib}, confirmed: ${confs}]",
                 ("p", producer)
                 ("id", fc::variant(pending->id).as_string().substr(0, 16))
                 ("n", pending->block_num)
                 ("t", pending->block->timestamp)
                 ("count", pending->block->transactions.size())
                 ("lib", db.last_irreversible_block_num())
                 ("confs", pending->header.confirmed)(capture));
            break;
        }
