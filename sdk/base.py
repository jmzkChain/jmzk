...
class Asset:
    def __init__(self, name, min_precision=0, max_precision=18):
        self.name = name
    ...
    def check(self, num_str):
        if '.' in num_str:
            ...
            precision = 0
        if precision > self.max_precision or precision < self.min_precision:
            raise AssetPrecisionException

def new_asset(name, **kwargs):
    asset = Asset(name, **kwargs)
    def value(num_str):
        asset.check(num_str)
        return num_str + " " + asset.name
    return value

EvtAsset = new_asset("EVT", min_precision=4, max_precision=4)
...
class PermissionDef(BaseType):
    ...
    def __init__(self, name, threshold, authorizers=[]):
        super().__init__(name=name, threshold=threshold, authorizers=[auth.dict() for auth in authorizers])
    def add_authorizer(self, auth, weight):
        self.kwargs["authorizers"].append(AuthorizerWeight(auth, weight).dict())
