import unittest
from pyevt import libevt, abi, ecc
import base, api, action, transaction
...
class Test(unittest.TestCase):
    ...
    def test_action_updatedomain(self):
        ...
        manage_per = base.PermissionDef("manage", threshold=3)
        manage_per.add_authorizer(ar1, weight=1)
        manage_per.add_authorizer(ar2, weight=1)

        updatedomain = AG.new_action("updatedomain", name=name, issuer=pub_key, manage=manage_per)
        trx = transaction.Transaction()
        ...
        resp = api.get_domain('{"name": "%s"}'%(name)).json()
        self.assertTrue({"ref": ar2.value,"weight": 1} in resp["manager"]["authorizers"])
    ''' 
    ...
    def test_action_newgroup(self):
        ...
        newgroup = AG.new_action("newgroup", name=name, key=pub_key)
    ...
    def test_action_updateowner(self):
        ...
        updateowner = AG.new_action("updateowner", name=name, owner=[pub1,pub2])
        trx = transaction.Transaction()
    ...
    def test_action_transferevt(self):
        ...
        # Each new account has 1.0000 EVT
        # Try to transfer 0.01 EVT from account1 to account2
        transferevt = AG.new_action("transferevt", _from=account1, to=account2, amount=EvtAsset("0.0100"))
        trx = transaction.Transaction()
        trx.add_action(transferevt)
        trx.add_sign(priv1) #sign with account1's owner's priv_key
        api.push_transaction(data=trx.dumps())

        # After Transfer, account1 should have 0.9900 EVT, and account2 should have 1.0100 EVT
        ac1 = api.get_account('''{"name": "%s"}'''%(account1)).json()["balance"]
        ac2 = api.get_account('''{"name": "%s"}'''%(account2)).json()["balance"]
        self.assertTrue(ac1 == "0.9900 EVT" and ac2 == "1.0100 EVT")
    ...
    def test_action_issuetoken(self):
        ...
        newdomain = AG.new_action("newdomain", name=domain_name, issuer=pub_key)
        issuetoken = AG.new_action("issuetoken", domain=domain_name, names=[token_name], owner=[pub_key])
    ...
    def test_action_transfer(self):
        ...
        pub2, priv2 = ecc.generate_new_pair()
        transfer = AG.new_action("transfer", domain=domain_name, name=token_name, to=[pub2])
        trx = transaction.Transaction()
