import json, pyevt
from pyevt import abi, libevt, ecc
import base
...
class ActionGenerator:
    def newdomain(self, name, issuer, **kwargs):
        ...
        abi_json = base.NewDomainAbi(name=name,
                issuer=str(issuer),
                issue=issue or base.PermissionDef("issue", 1, [auth_A]),
                transfer=transfer or base.PermissionDef("transfer", 1, [auth_G]),
                manage=manage or base.PermissionDef("manage", 1, [auth_A]))
        return get_action_from_abi_json("newdomain", abi_json.dumps())

    def updatedomain(self, name, issuer, **kwargs):
        auth_A = base.AuthorizerWeight(base.AuthorizerRef("A", issuer), 1)
        auth_G = base.AuthorizerWeight(base.AuthorizerRef("G", "OWNER"), 1)
        issue = kwargs["issue"] if "issue" in kwargs else None
        transfer = kwargs["transfer"] if "transfer" in kwargs else None
    ...
    def newgroup(self, name, key, **kwargs):
        group = kwargs["group"] if "group" in kwargs else None
        if group == None:
            pub1, priv1 = ecc.generate_new_pair()
            pub2, priv2 = ecc.generate_new_pair()
            ln1 = base.LeafNode(key=str(pub1), weight=1)
            ln2 = base.LeafNode(key=str(pub2), weight=1)
            ln3 = base.LeafNode(key=str(pub2), weight=3)
            nln1 = base.NonLeafNode(threshold=1, weight=3, nodes=[])
            nln1.add_child(ln1)
            nln1.add_child(ln2)
            nln2 = base.NonLeafNode(threshold=1, weight=3, nodes=[ln1,ln2])
            root = base.RootNode(threshold=6, nodes=[nln1, ln2, nln2])
            group = base.Group(name=name, key=str(key), root=root.dict())
        abi_json = base.NewGroupAbi(name=name, group=group.dict())
        return get_action_from_abi_json("newgroup", abi_json.dumps())

    def updategroup(self, name, key, **kwargs):
        group = kwargs["group"] if "group" in kwargs else None
        if group == None:
            pub1, priv1 = ecc.generate_new_pair()
            pub2, priv2 = ecc.generate_new_pair()
            ln1 = base.LeafNode(key=pub1, weight=1)
            ln2 = base.LeafNode(key=pub2, weight=1)
            ln3 = base.LeafNode(key=pub2, weight=3)
            nln1 = base.NonLeafNode(threshold=1, weight=3, nodes=[])
            nln1.add_child(ln1)
            nln1.add_child(ln2)
            nln2 = base.NonLeafNode(threshold=1, weight=3, nodes=[ln1,ln2])
            root = base.RootNode(threshold=6, nodes=[nln1, ln2, nln2])
            group = base.Group(name="group0619", key=str(key), root=root.dict())
        abi_json = base.NewGroupAbi(name=name, group=group.dict())
        return get_action_from_abi_json("updategroup", abi_json.dumps())

    def newaccount(self, name, owner):
        abi_json = base.NewAccountAbi(name=name, owner=[str(each) for each in owner])
        return get_action_from_abi_json("newaccount", abi_json.dumps())

    def updateowner(self, name, owner):
        abi_json = base.UpdateOwnerAbi(name=name, owner=[str(each) for each in owner])
    ...
    def issuetoken(self, domain, names, owner):
        abi_json = base.IssueTokenAbi(domain, names, owner=[str(each) for each in owner])
        return get_action_from_abi_json("issuetoken", abi_json.dumps())

    def transfer(self, domain, name, to):
        abi_json = base.TransferAbi(domain, name, to=[str(each) for each in to])
